...
let veryOldInventors = inventors.filter((val, key) => {
  return val.year > 1499 && val.year < 1600
})
...
let inventorNames = inventors.map((val, key, arr) => {
...
let inventorNamedAda = inventors.find((person) => {
  return person.first == 'Ada'
})
...
let commentById = comments.find((val) => {
...
let idx = comments.findIndex((val) => {
  return val.id == 123523
})
...
let totalYearsLived = inventors.reduce((acc, cv) => {
  return (acc += cv.passed - cv.year)
}, 0)
